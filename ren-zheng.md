---
description: 本文档所有接口使用 API Key 作为 Bearer Token 认证。本页说明 Key 的获取、传递、管理与常见问题。
hidden: true
---
...
curl -X POST https://api-m.novada.com/v1/wallet/balance \ -H "Authorization: Bearer YOUR\_API\_KEY"
...
curl -X POST https://api-m.novada.com/v1/wallet/balance \ -H "Authorization: Bearer YOUR\_API\_KEY"COMPATIBLE

#### Token Exchange（兼容保留）

先调用 `/v1/oauth2/token` 换取 access\_token（7 天 TTL）再调业务接口。

curl -X POST .../v1/oauth2/token \ -H "Authorization: Bearer YOUR\_API\_KEY" # 老 Basic Auth (username:api\_key) 兼容保留
...
4.  **复制并保存**
    ...
    保存到密码管理器或 secret 存储（避免明文进代码仓库）。Key 可在 Dashboard 随时查看与复制。

### API Key 管理

本节中涉及的两个错误码（`401 api_key_disabled` / `402 product_not_subscribed`）、生效行为（立即生效）、套餐解耦行为。

| 入口    | <p><a href="https://dashboard.novada.com/cn/api-key/">dashboard.novada.com/cn/api-key/</a><br>（页面实际渲染时根据当前文档语言切换：中文版渲染 <code>/cn/api-key/</code>，英文版渲染 <code>/api-key/</code>，不再两个并列。）</p>                                                                        |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 数量    | 单账号最多 **10 个** API Key，可独立启停。                                                                                                                                                                                                                                     |
| 有效期   | 默认最长 5 年，**可设永不过期**。Dashboard 内可随时修改。                                                                                                                                                                                                                             |
...
API Key 与账号密码同等敏感。一旦怀疑泄露（误提交到 Git、被同事意外看到、设备遗失等），请立即在 [Dashboard](https://dashboard.novada.com/cn/api-key/) 删除旧 Key 并新建一个，再把生产环境的引用替换为新 Key。
...
<summary>能否在 Dashboard 查看已有 API Key 的明文？</summary>
