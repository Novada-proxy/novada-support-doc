...
| 覆盖范围    | <p>一 Key 覆盖：开放接口、Scraper API、Web Unblocker 全部接口（均为 HTTP/HTTPS，统一通过 <code>Authorization: Bearer YOUR_API_KEY</code> 传递）。<br>⚠️ <strong>Browser API 实际浏览器接入</strong>（WebDriver / WSS）使用 <code>username:password</code> 凭证，不在 API Key 覆盖范围内（详见 <a href="https://www.novada.com/cn/products/browser-api/">Browser API 产品页</a>）。</p> |
